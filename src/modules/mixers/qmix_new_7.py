...
class QMixer_new(nn.Module):
    def __init__(self, args):
        ...
        self.hyper_w_1 = nn.Linear(self.action_dim, self.n_agents * self.embed_dim_2)
        self.hyper_b_1 = nn.Linear(self.action_dim, self.embed_dim_2)
    ...
    def forward(self, agent_qs, states, actions):
        ...
        w1 = w1.view(-1, self.n_agents, self.embed_dim_2)
        b1 = b1.view(-1, 1, self.embed_dim_2)
        hidden = F.elu(th.bmm(agent_qs, w1) + b1)
        # Second layer(action)
        w2 = th.abs(self.hyper_w_2(states))
        b2 = self.hyper_b_2(states)
        w2 = w2.view(-1,self.embed_dim_2,self.embed_dim)
